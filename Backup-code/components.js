// Fungsi untuk nge-render Navbar
    ...
    <nav class="fixed top-0 left-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
          ...
          <div class="hidden md:flex items-center justify-center gap-8 flex-1 mr-12" id="main-nav-links">
            <a href="index.html#apps" class="text-sm font-medium text-gray-600 hover:text-primary transition-colors" onclick="showHome(event)">Aplikasi</a>
            <a href="index.html#categories" class="text-sm font-medium text-gray-600 hover:text-primary transition-colors" onclick="showHome(event)">Kategori</a>
            <a href="index.html#pricing" class="text-sm font-medium text-gray-600 hover:text-primary transition-colors" onclick="showHome(event)">Harga</a>
            <a href="index.html#support" class="text-sm font-medium text-gray-600 hover:text-primary transition-colors" onclick="showHome(event)">Bantuan</a>
          </div>
          ...
          <div class="hidden lg:flex flex-1 max-w-md relative">
            ...
            <!-- Menggunakan event onkeydown untuk mengecek tombol Enter -->
            <input type="text" id="navbar-search" onkeydown="handleSearch(event)" class="block w-full pl-10 pr-3 py-2.5 border border-gray-200 rounded-full leading-5 bg-gray-50 placeholder-gray-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors sm:text-sm" placeholder="Cari aplikasi... (Tekan Enter)" />
          </div>

          <!-- Kanan: Ikon & Auth -->
          <div class="flex items-center justify-end gap-5 ml-6">
    ...
    <div id="cart-sidebar" class="fixed top-0 right-0 h-full w-full sm:w-[450px] bg-white z-[110] transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col shadow-2xl">
      ...
      <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-white z-10">
          <div class="flex items-center gap-2">
              ...
              <h2 class="text-lg font-bold text-gray-900">Keranjang Anda</h2>
          </div>
          <button onclick="closeCart()" class="w-8 h-8 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-full flex items-center justify-center transition-colors">
      ...
      <div id="cart-items-container" class="flex-grow overflow-y-auto p-6 flex flex-col gap-4 custom-scrollbar bg-surface">
          <!-- Items akan di-render di sini secara dinamis -->
      </div>
      ...
      <div class="border-t border-gray-100 p-6 bg-white z-10 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]">
          ...
          <button class="w-full bg-primary hover:bg-primaryHover text-white font-bold rounded-2xl py-4 flex items-center justify-center gap-2 transition-colors shadow-lg shadow-primary/30">
         ...
         <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div class="lg:col-span-1">
                ...
                <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col gap-1">
                    <button id="btn-akun-saya" onclick="switchProfileTab('akun-saya')" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-primary/5 text-primary font-medium transition-colors text-left w-full">
                        <i data-lucide="package" class="w-5 h-5"></i> Akun Saya
                    </button>
                    <button id="btn-riwayat" onclick="switchProfileTab('riwayat')" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium transition-colors text-left w-full">
                        <i data-lucide="clock" class="w-5 h-5"></i> Riwayat Transaksi
                    </button>
                    <button id="btn-pengaturan" onclick="switchProfileTab('pengaturan')" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium transition-colors text-left w-full">
                        <i data-lucide="settings" class="w-5 h-5"></i> Pengaturan
                    </button>
                    <hr class="my-2 border-gray-100">
                    <button onclick="showHome(event)" class="flex items-center gap-3 px-4 py-3 rounded-xl text-red-500 hover:bg-red-50 font-medium transition-colors text-left w-full">
            ...
            <div class="lg:col-span-3 space-y-6">
                <!-- Stats Overview (Selalu tampil di atas) -->
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
                        <div class="w-12 h-12 rounded-full bg-orange-50 text-orange-500 flex items-center justify-center flex-shrink-0">
                            <i data-lucide="ticket" class="w-6 h-6"></i>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 font-medium">Voucher Tersedia</p>
                            <h3 class="text-2xl font-bold text-gray-900">2</h3>
                        </div>
                     </div>
                     <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4">
                        <div class="w-12 h-12 rounded-full bg-yellow-50 text-yellow-600 flex items-center justify-center flex-shrink-0">
                            <i data-lucide="coins" class="w-6 h-6"></i>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500 font-medium">AppStore Poin</p>
                ...
                <!-- TAB 1: AKUN SAYA -->
                <div id="content-akun-saya" class="block animate-modal-enter">
                  <div class="flex items-center justify-between mt-8 mb-4">
                      <h3 class="text-xl font-bold text-gray-900">Detail Akun Aktif</h3>
                  </div>

                  <div class="space-y-6">
                      <!-- Akun 1: Streaming Sharing -->
                              ...
                              <div class="flex-grow">
                                  <div class="mb-4">
                                      ...
                                      <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                                          <p class="text-gray-500">Jenis: <span class="font-medium text-gray-900">1P1U (Sharing)</span></p>
                                          <span class="w-1 h-1 rounded-full bg-gray-300"></span>
                                          <p class="text-gray-500">Durasi: <span class="font-medium text-gray-900">1 Bulan</span></p>
                                          <span class="w-1 h-1 rounded-full bg-gray-300"></span>
                                  ...
                                  <div class="bg-gray-50 rounded-xl p-4 grid grid-cols-1 md:grid-cols-2 gap-4 border border-gray-100">
                                      <div>
                                          <p class="text-xs text-gray-500 mb-1.5 font-medium uppercase tracking-wider">Email Akun</p>
                                          <div class="flex items-center justify-between bg-white px-3 py-2.5 rounded-lg border border-gray-200 shadow-sm">
                                              <span class="text-sm font-mono text-gray-800 truncate">netflix01@appstorehub.com</span>
                                              <button onclick="copyToClipboard('netflix01@appstorehub.com')" class="text-gray-400 hover:text-primary transition-colors flex-shrink-0 ml-2" title="Salin Email">
                                                  <i data-lucide="copy" class="w-4 h-4"></i>
                                              </button>
                                          </div>
                                      </div>
                                      <div>
                                          <p class="text-xs text-gray-500 mb-1.5 font-medium uppercase tracking-wider">Password</p>
                                          <div class="flex items-center justify-between bg-white px-3 py-2.5 rounded-lg border border-gray-200 shadow-sm">
                                              <span class="text-sm font-mono text-gray-800 truncate">NtfBudi!23</span>
                                              <button onclick="copyToClipboard('NtfBudi!23')" class="text-gray-400 hover:text-primary transition-colors flex-shrink-0 ml-2" title="Salin Password">
                                                  <i data-lucide="copy" class="w-4 h-4"></i>
                                              </button>
                                          </div>
                                      </div>
                                      <div>
                                          <p class="text-xs text-gray-500 mb-1.5 font-medium uppercase tracking-wider">Profil Kamu</p>
                                          <div class="flex items-center justify-between bg-white px-3 py-2.5 rounded-lg border border-gray-200 shadow-sm">
                                              <span class="text-sm font-medium text-gray-800 truncate">Profil 3 - Budi</span>
                                          </div>
                                      </div>
                                      <div>
                                          <p class="text-xs text-gray-500 mb-1.5 font-medium uppercase tracking-wider">PIN Profil</p>
                                          <div class="flex items-center justify-between bg-white px-3 py-2.5 rounded-lg border border-gray-200 shadow-sm">
                                              <span class="text-sm font-mono text-gray-800 truncate">1234</span>
                                              <button onclick="copyToClipboard('1234')" class="text-gray-400 hover:text-primary transition-colors flex-shrink-0 ml-2" title="Salin PIN">
                                                  <i data-lucide="copy" class="w-4 h-4"></i>
                                              </button>
                                          </div>
                                      </div>
                          ...
                          <div class="flex flex-col md:flex-row md:items-start gap-6">
                              ...
                              <div class="flex-grow">
                                  <div class="mb-4">
                                      ...
                                      <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                                          <p class="text-gray-500">Jenis: <span class="font-medium text-gray-900">Private</span></p>
                                          <span class="w-1 h-1 rounded-full bg-gray-300"></span>
                                          <p class="text-gray-500">Durasi: <span class="font-medium text-gray-900">1 Bulan</span></p>
                                          <span class="w-1 h-1 rounded-full bg-gray-300"></span>
                ...
                <div id="content-riwayat" class="hidden animate-modal-enter">
                  <div class="flex items-center justify-between mt-8 mb-4">
                ...
                <!-- TAB 3: PENGATURAN -->
                <div id="content-pengaturan" class="hidden animate-modal-enter">
                  <div class="flex items-center justify-between mt-8 mb-4">
                      <h3 class="text-xl font-bold text-gray-900">Pengaturan Akun</h3>
                  </div>
                  <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                      <form onsubmit="event.preventDefault(); alert('Profil berhasil diperbarui!');" class="space-y-5 max-w-lg">
                          <div>
